...
while read -r message; do
    if [ "$message" = "TERMINATE" ]; then
        ...
        pkill -INT -f "/chain-adjacent-service"
        ...
        echo "=== CAS PID ==="
        cas_pid=$(pgrep -f "/chain-adjacent-service" || echo "none")
        if [ "$cas_pid" = "none" ]; then
            echo "chain-adjacent-service not running"
        else
            echo "$cas_pid"
            echo "=== CAS MEM ==="
            ps -p "$cas_pid" -o rss --no-headers | awk '{printf "Memory Used: %.2f MB\n", $1/1024}'
            echo "=== CAS STATE ==="
            ps -p "$cas_pid" -o state,cmd --no-headers || echo "No state info"
            echo "=== CAS CPU ==="
            ps -p "$cas_pid" -o %cpu --no-headers | awk '{printf "CPU Used: %.2f%%\n", $1}'
        fi
        echo "=== NETSTAT ==="
        netstat -tunap | grep -E '9642|9650|2049'
