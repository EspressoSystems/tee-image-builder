...
echo "Setting up CAS config directory..."
mkdir -p /opt/cas/config || { echo "Failed to create config directory"; exit 1; }
sudo chown -R ec2-user:ec2-user /opt/cas/config || { echo "Failed to set permissions for config"; exit 1; }
...
echo "/opt/cas/config 127.0.0.1/32(ro,insecure,crossmnt,no_subtree_check,sync,all_squash,anonuid=1000,anongid=1000)" | sudo tee -a /etc/exports || { echo "Failed to configure NFS exports"; exit 1; }
